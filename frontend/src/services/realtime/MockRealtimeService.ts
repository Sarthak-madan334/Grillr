...
export class MockRealtimeService {
  ...
  connect() {
    ...
    setTimeout(() => {
      this.simulateAIQuestion('Tell me about yourself and your experience.', 'https://mock-audio-url.com');
    }, 1000);
  }
  ...
  simulateAIQuestion(text: string, audioUrl?: string) {
    ...
    this.emit({ type: 'question.created', data: { text, audio_url: audioUrl } });
  ...
  simulateFinalUserSpeech(finalText: string) {
    this.emit({ type: 'transcript.final', data: { text: finalText, answer_id: 'mock-uuid' } });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        this.simulateAIQuestion('That makes sense. Can you dive deeper into a time you faced a difficult challenge?');
      }, 1000);
    }, 2000);
  }
  ...
  clearMocks() {
    this.mockIntervals.forEach((id: any) => window.clearInterval(id));
    this.mockIntervals = [];
  }
}

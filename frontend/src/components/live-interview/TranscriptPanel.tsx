...
export function TranscriptPanel({ entries }: TranscriptPanelProps) {
  return (
    <section className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-[0_14px_32px_rgba(15,23,42,0.04)] md:p-5" aria-live="polite">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-[0.24em] text-slate-400">Transcript</div>
          <h3 className="mt-1 text-lg font-semibold text-slate-900">Live response</h3>
        </div>
              ...
              >
                <div
                  className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold uppercase tracking-[0.18em] ${
                    isAi ? "bg-slate-900 text-white" : "bg-indigo-600 text-white"
                  }`}
                >
                  {isAi ? "AI" : "You"}
                </div>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p
                    className={`text-sm leading-6 ${
                      entry.isPartial ? "italic text-slate-500" : "text-slate-700"
                    }`}
                  >
                    {entry.text}
                  </p>

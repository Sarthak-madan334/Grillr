      ...
      <div className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-7xl overflow-hidden rounded-[30px] border border-[#d2d2d7] bg-white shadow-[0_28px_100px_rgba(0,0,0,0.1)] sm:min-h-[calc(100vh-3rem)] lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <section className="px-6 py-8 sm:px-12 sm:py-10 lg:px-16 lg:py-12">
          ...
          <div className="mx-auto mt-10 max-w-md lg:mt-14">
            ...
            <h2 className="mt-3 text-3xl font-semibold tracking-tight text-[#1d1d1f]">Create your account</h2>
            <p className="mt-3 text-sm leading-6 text-[#6e6e73]">Your next stronger answer starts here.</p>
            <div className="mt-7 grid gap-3"><button type="button" className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-full border border-[#d2d2d7] bg-white text-sm font-medium text-[#1d1d1f] transition hover:-translate-y-0.5 hover:bg-[#f5f5f7]"><GoogleIcon />Continue with Google</button><button type="button" className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-full border border-[#d2d2d7] bg-white text-sm font-medium text-[#1d1d1f] transition hover:-translate-y-0.5 hover:bg-[#f5f5f7]"><GithubIcon />Continue with GitHub</button></div>
            <div className="my-7 flex items-center gap-3 text-xs uppercase tracking-[0.18em] text-[#6e6e73]"><span className="h-px flex-1 bg-[#d2d2d7]" />or<span className="h-px flex-1 bg-[#d2d2d7]" /></div>
            ...
            <form className="space-y-5" onSubmit={handleSubmit} noValidate aria-busy={isSubmitting}><div className="grid gap-5 sm:grid-cols-2"><div className="space-y-2"><label htmlFor="firstName" className="text-sm font-medium text-[#424245]">First name</label><Input id="firstName" placeholder="Ari" value={values.firstName} onChange={(event) => setValues({ ...values, firstName: event.target.value })} disabled={isSubmitting} aria-invalid={Boolean(errors.firstName)} /></div><div className="space-y-2"><label htmlFor="lastName" className="text-sm font-medium text-[#424245]">Last name</label><Input id="lastName" placeholder="Miller" value={values.lastName} onChange={(event) => setValues({ ...values, lastName: event.target.value })} disabled={isSubmitting} aria-invalid={Boolean(errors.lastName)} /></div></div><div className="space-y-2"><label htmlFor="email" className="text-sm font-medium text-[#424245]">Email</label><Input id="email" type="email" placeholder="you@example.com" value={values.email} onChange={(event) => setValues({ ...values, email: event.target.value })} disabled={isSubmitting} aria-invalid={Boolean(errors.email)} /></div><div className="space-y-2"><label htmlFor="password" className="text-sm font-medium text-[#424245]">Password</label><Input id="password" type="password" placeholder="Create a strong password" value={values.password} onChange={(event) => setValues({ ...values, password: event.target.value })} disabled={isSubmitting} aria-invalid={Boolean(errors.password)} /></div><Button className="w-full" size="lg" disabled={isSubmitting}>{isSubmitting ? "Creating account..." : "Create account"}</Button></form>
          </div>
        </section>
      </div>
    </main>
